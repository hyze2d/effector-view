...
type ConnectorBuilderResult<P, MP, K extends string> = Omit<
  ViewConnectorBuilder<P, MP, K>,
  K
>;

type View<P, MP> = ((props: Omit<P, keyof MP>) => JSX.Element | null) & {
  defaultProps?: Partial<P>;

  displayName?: string;

  memo: MemoExoticComponent<
    (props: Omit<P, keyof MP> & MP) => JSX.Element | null
  >;
};
...
type ViewBuilder<P, MP, K extends string> = BaseBuilder<P, MP, K> & {
  ...
  view: (
    render: (props: Omit<P, keyof MP> & MP) => JSX.Element | null
  ) => View<P, MP>;
};

type ViewConnectorBuilder<P, MP, K extends string> = BaseBuilder<P, MP, K> & {
  props: <
    T extends Partial<{
  ...
  >(
    props: T
  ) => ConnectorBuilderResult<P, MP & MapStoresToValues<T>, K | 'props'>;

  map: <T extends Partial<P>>(
    map: (props: P & MP) => T
  ) => ConnectorBuilderResult<P, MP & T, K | 'map'>;

  view: () => View<P, MP>;
};

type BaseConfig<P, MP> = {
  defaultProps?: Partial<P>;
  displayName?: string;
  enter?: Event<void>;
  exit?: Event<void>;
...
type Config<P, MP> = BaseConfig<P, MP> & {
